...
class TestGeneratedCoreDivisions(BaseTestScratchpadUsage):
    ...
    *live* ops, which is what a generated division rests on."""
        ...
        for buf in buffers:
            ...
            for parent, edge in buf.residency_edges.items():
                ...
                for i, parent_division in enumerate(parent_divisions):
                    ...
                    # Identity, not only membership. The two relations need not
                    # pick the same compatible entry -- the table takes the
                    # lowest menu position, the inverse the first solution its
                    # own ordering reaches -- so which one each row got is
                    # recorded, and the test pins that they still coincide here.
                    same = constructed.label == labels[0]
                    checked.append(("inverse" if same else "inverse-differs", buf.name))
    ...
    def test_generated_divisions_agree_with_the_enumeration_on_a_real_graph(self):
        ...
        # The inverse and the pair table agree on every row of this graph, which
        # membership alone could not tell from the two diverging.
        self.assertNotIn(
            "inverse-differs",
            kinds,
            "the inverse no longer returns the table's lowest-position pick; "
            "that is allowed (see _ViewRelation) but it changes what a flood "
            "propagates, so it should be a deliberate change",
        )
        self.assertTrue(
            torch.allclose(
