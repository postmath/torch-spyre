...
class _DivisionSource:
    ...
    def retiled(
        ...
        back to untiled. Splits are held fixed, so
...
class SaCoOptimizingSolver(CoreDivisionLayoutSolver):
    ...
    def _precompute_topology(self) -> None:
        ...
        # copy idx -> (source idx or None, the readers with an operation
        # position, their positions, the conflicting ones).
        self._read_copy_topology: dict[
            int,
            tuple[Optional[int], tuple[int, ...], tuple[int, ...], frozenset[int]],
        ] = {}
        ...
        at_position = {
            b.op_position: i for i, b in enumerate(bufs) if b.op_position is not None
        }
        for idx, buf in enumerate(bufs):
            if not isinstance(buf, CoarseTileReadCopyBuffer):
                continue
            source_idx = self._name_to_idx.get(buf.source)
            placed = [
                (r, at)
                for name in buf.readers
                if (at := bufs[r := self._name_to_idx[name]].op_position) is not None
            ]
            readers = tuple(r for r, _ in placed)
            positions = tuple(at for _, at in placed)
            conflicting = frozenset(self._name_to_idx[name] for name in buf.conflicting)
            self._read_copy_topology[idx] = (
                source_idx,
                readers,
                positions,
                conflicting,
            ...
            watched = {buf.reader_index, *readers}
            watched.update(
                at_position[at] for at in range(first, last + 1) if at in at_position
            )
            if source_idx is not None:
                watched.add(source_idx)
            for watcher in sorted(watched):
                self._read_copies_of[watcher].append(idx)
            self._read_copies_sized_by[buf.reader_index].append(idx)
    ...
    def _eligible(self, idx: int, runs: Optional[tuple[int, list[int]]] = None) -> bool:
    ...
    def _read_across_a_tiling_boundary(self, idx: int, parent: DivisionConfig) -> bool:
        ...
        * ``P`` produced by an **untiled** op, or by no op at all (a graph input's
        ...
        be resident where its source cannot. The solve places it as a
        :class:`CoarseTileReadCopyBuffer`.
        """
        ...
        while start < self._n_positions:
            ...
            if not spec.is_untiled:
                cuts = [
                    ...
                    if self._breaks_at(at, start, spec)
                ]
                for lo, hi in zip([start, *cuts], [at - 1 for at in cuts] + [end]):
    ...
    def _staged_reads(
        self, idx: int, runs: Optional[tuple[int, list[int]]] = None
    ) -> int:
        ...
        """
        buf = self._bufs[idx]
        reader = buf.reader_index
        if self.chosen[reader].tiling.is_untiled:
            return 0
        source_idx, readers, positions, conflicting = self._read_copy_topology[idx]
        if reader not in readers:
            return 0
        if source_idx is not None and not self.chosen[source_idx].tiling.is_untiled:
            return 0
        if runs is None:
            runs = self._runs_over([idx])
        base, run_of = runs
        run = run_of[self._position_of[reader] - base]
        in_group = [r for r, at in zip(readers, positions) if run_of[at - base] == run]
        if not in_group or in_group[0] != reader or conflicting.intersection(in_group):
            return 0
        # One read gains nothing: the copy op itself makes it.
        return len(in_group) if len(in_group) > 1 else 0

    def _runs_over(self, copies: Iterable[int]) -> tuple[int, list[int]]:
        """``(base, run)``: ``run[p - base]`` numbers the tiling run holding
        position ``p``, over the spans of ``copies`` -- all
        :meth:`_staged_reads` compares, so a ripple builds it once.

        Exact between readers although the scan may start mid-stretch: a
        break there tests only reads of positions the span reaches back to.
        """
        spans = [self._read_copy_span[c] for c in copies if c in self._read_copy_span]
        if not spans:
            return 0, []
        lo, hi = min(span[0] for span in spans), max(span[1] for span in spans)
        run_of = []
        run, spec, stretch = 0, self._tiling_at(lo), lo
        for position in range(lo, hi + 1):
            at = self._tiling_at(position)
            if at != spec:
                run, spec, stretch = run + 1, at, position
            elif self._breaks_at(position, stretch, spec):
                run += 1
            run_of.append(run)
        return lo, run_of
    ...
    def _trim_tilings_to_anchor_run(
        ...
        extent, through a companion buffer (priced by :meth:`_companion_bytes`).
        A recolor tiles the anchor's run only; this strips tilings and moves
        nothing else. The divisions are deliberately untouched -- they are what
        the flood is for, and narrowing them to the run would cost the
        long-range division move stage 2b measured at -0.71%.
