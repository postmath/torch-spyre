...
best score. It then writes a markdown report + plots to ``docs/source/user_guide/examples/scratchpad/``.

Run from the repo root (so ``tests`` is importable)::

    python3 docs/source/user_guide/examples/scratchpad/profile_coopt_schedule_sweep.py            # sweep + report
    python3 docs/source/user_guide/examples/scratchpad/profile_coopt_schedule_sweep.py --report   # report only
...
REPORT_MD = os.path.join(_REPORTS, "coopt_schedule_sweep.md")
CROSSOVER_PNG = os.path.join(_IMAGES, "coopt_schedule_crossover.png")
GRID_PNG = os.path.join(_IMAGES, "coopt_schedule_per_graph.png")
...
def write_report():
    ...
    out.append(
        f"![crossover](../../_static/images/coopt/{os.path.basename(CROSSOVER_PNG)})\n"
    )
    out.append(
        f"![per-graph](../../_static/images/coopt/{os.path.basename(GRID_PNG)})\n"
    )
